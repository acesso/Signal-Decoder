      ...
      struct cdecode *xprevdecs)
{
  ...
  for(int i = 0; i < nthreads; i++){
    ...
    // No SharedArrayBuffer/pthreads on GitHub Pages; the caller already runs
    // inside a Web Worker, so decode synchronously (use nthreads=1).
    ft8->th_ = nullptr;
    ft8->go(npasses);
#else
...
#ifdef __EMSCRIPTEN__
  // Don't kill the whole WASM instance over a bad param name from JS.
  return nan("");
#else
  exit(1);
  return 0;
