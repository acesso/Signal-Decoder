// LDPC(174,91) parity-check matrix — verbatim transcription of Nm/Mn from
// lib/ft8mon/arrays.h (WSJT-X's ldpc_174_91_c_reordered_parity.f90), kept
// 1-origin exactly as in the C source (index 0 = "unused" sentinel in Nm's
// padding slots) so this stays trivially diffable against the C array
// literals if ft8mon's own copy ever changes.
//
// Nm[j] (83 rows, padded to 7): codeword-bit indices (1-origin, 0=unused)
// participating in parity check j. Check-node degree <= 7 (irregular).
// Mn[i] (174 rows, exactly 3): parity-check indices (1-origin) that codeword
// bit i participates in. Variable-node degree == 3 (regular).
export const LDPC_N = 174; // codeword bits
export const LDPC_K = 91; // systematic payload bits
export const LDPC_CHECKS = 83; // parity checks (174-91)
export const NM_MAX_DEGREE = 7;
export const MN_DEGREE = 3;
...
if (Nm.length !== LDPC_CHECKS) throw new Error(`Nm must have ${LDPC_CHECKS} rows, got ${Nm.length}`);
if (Mn.length !== LDPC_N) throw new Error(`Mn must have ${LDPC_N} rows, got ${Mn.length}`);

/** Flattened, 0-origin, row-major versions for WGSL storage buffers (WGSL
 *  has no 2D array literal upload path — flat arrays with a fixed stride
 *  are the natural fit). Sentinel 0 (from Nm's padding) becomes -1 so a
 *  WGSL `if (idx >= 0)` check reads the same as the C `if(i1 >= 0)` guard. */
export function flattenNm(): Int32Array {
  const out = new Int32Array(LDPC_CHECKS * NM_MAX_DEGREE);
  for (let j = 0; j < LDPC_CHECKS; j++) {
    for (let k = 0; k < NM_MAX_DEGREE; k++) {
      const v = Nm[j][k];
      out[j * NM_MAX_DEGREE + k] = v === 0 ? -1 : v - 1;
    }
  }
  return out;
}

export function flattenMn(): Int32Array {
  const out = new Int32Array(LDPC_N * MN_DEGREE);
  for (let i = 0; i < LDPC_N; i++) {
    for (let k = 0; k < MN_DEGREE; k++) {
      out[i * MN_DEGREE + k] = Mn[i][k] - 1;
    }
  }
  return out;
}
